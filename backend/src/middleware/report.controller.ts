...
export async function getDailyLedger(req: Request, res: Response) {
  ...
  const [collections, receipts, routes] = await Promise.all([
    ...
    // FactoryReceipt links via graderId — join through Employee→Route
    prisma.factoryReceipt.findMany({
      where: { receivedAt: { gte: d, lt: next } },
      select: { graderId: true, litres: true, grader: { select: { routeId: true } } },
      include: { grader: { select: { routeId: true } } } as any,
    }).catch(() => [] as any[]),
